...
TEST_CASE("task of reference type")
{
    ...
    cb::sync_wait(
        [&]() -> cb::task<>
        {
            ...
            SECTION("joining rvalue task")
            {
                decltype(auto) result = cb::sync_wait(f());
                static_assert(std::is_same<decltype(result), int&>::value,
                              "joined r-value reference of task<int&> should result in an int&");
                CHECK(&result == &value);
            }
            ...
            SECTION("joining lvalue task")
            {
                auto t = f();
                decltype(auto) result = cb::sync_wait(t);
                static_assert(std::is_same<decltype(result), int&>::value,
                              "joined l-value reference of task<int&> should result in an int&");
                CHECK(&result == &value);
            }
        }());
}
...
TEST_CASE("Return value is moved on co_await")
{
    ...
    cb::sync_wait(
        [&]() -> cb::task<>
        {
            ...
            auto result3 = co_await makeTask();
...
TEST_CASE("task<>.join() test")
{
    ...
    auto task = makeTask();
    REQUIRE(cb::sync_wait(task) == "foo");

    REQUIRE(cb::sync_wait(makeTask()) == "foo");
}
...
TEST_CASE("task<>.join() test: multiple threads")
{
    ...
    {
        CHECK(cb::sync_wait(createTask()) == i);
    }
}
