...
struct wsabuf
{
    ...
    constexpr wsabuf(void* ptr, std::size_t size)
        : len(size <= ulong_t(-1) ? ulong_t(size) : ulong_t(-1)), buf(static_cast<char*>(ptr))
    {
    }
...
{
    using callback_type = void(io_state* state, win32::dword_t errorCode, win32::dword_t numberOfBytesTransferred,
...
class safe_handle
{
   public:
    ...
    bool operator==(const safe_handle& other) const { return m_handle == other.m_handle; }
