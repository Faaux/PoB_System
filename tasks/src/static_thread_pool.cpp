...
public:
	explicit thread_state()
		: m_localQueue(
			std::make_unique< std::atomic< schedule_operation* >[] >( local::initial_local_queue_size ) )
	...
	{
		return difference( m_head.load( std::memory_order_relaxed ),
...
void static_thread_pool::remote_enqueue( schedule_operation* operation ) noexcept
{
	...
	do {
		...
		tail, operation, std::memory_order_seq_cst, std::memory_order_relaxed ) );
}
...
void static_thread_pool::try_clear_intent_to_sleep( std::uint32_t threadIndex ) noexcept
{
	...
	do {
		...
		oldSleepingCount, oldSleepingCount - 1, std::memory_order_acquire, std::memory_order_relaxed ) );
